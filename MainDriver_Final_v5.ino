...
void driveAroundObstacle(float tInit, float xInit, float yInit) {
  ...
  driveForward(0.4 * ms_m); // Drive N or S 35 cm
  enes.println("Drove north / south 40 cm");
  while (!updateCoordOSV()) {
    //loop until coordinates are updated
  }
  delay(1000);
  ...
  while (checkForObstacles()) {
    ...
    while (max(tInit, theta) - min(theta, tInit) > rad_tolerance) {
      timedTurn(tInit);
      while (!updateCoordOSV()) {
        //loop until coordinates are updated
      }
...
float turnToAvoidObstacle() {
  ...
  if (yPos > 1) {
    while (max(tS, theta) - min(theta, tS) > rad_tolerance) {
      timedTurn(tS);
      while (!updateCoordOSV()) {
        //loop until coordinates are updated
      }
    ...
    while (max(tN, theta) - min(theta, tN) > rad_tolerance) {
      timedTurn(tN);
      while (!updateCoordOSV()) {
        //loop until coordinates are updated
      }
